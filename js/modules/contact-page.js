...
class ContactForm {
  ...
  init() {
    ...
    emailjsLib.init(this.emailConfig.publicKey);
  ...
  initCharCounter() {
    ...
    const max = parseInt(this.textarea.getAttribute('maxlength') || '2000', 10);
  ...
  showStatus(type, message) {
    ...
    if (type === 'success') {
      window.setTimeout(() => this.clearStatus(), 8000);
    }
  }
...
function initCopyEmail() {
  ...
  if (!card || !label || !copyIco || !checkIco) return;
  ...
  card.addEventListener('click', (event) => {
    if (isCopyActionTarget(event.target)) {
      triggerCopy(event);
      return;
    }
    openEmailClient();
