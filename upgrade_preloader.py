...
def upgrade():
  ...
  preloader.innerHTML = `
    ...
    <div class="preloader-content">
      <div class="pl-glitch-box">
        ...
        <div class="preloader-log" id="pl-log3"></div><span class="pl-cursor"></span>
      </div>
  ...
  style.innerHTML = `
    ...
    .pl-scanline {
      ...
      z-index: 3; opacity: 0.6; animation: scanDown 2s linear infinite; pointer-events: none;
    ...
    /* Slight chromatic aberration entrance */
  ...
  const logs = [
    ...
    { el: document.getElementById('pl-log2'), text: "[DECRYPTING MODULES] SECURE", delay: 600 },
    { el: document.getElementById('pl-log3'), text: "[UI.DRAW] INITIATING RENDER LAYER", delay: 1000 }
  ];
  ...
  const fillInterval = setInterval(() => {
    fillPct += (Math.random() * 12 + 2);
    if (fillPct > 98) fillPct = 98; // hold at 98% until very end
    fill.style.width = fillPct + '%';
  }, 80);

  logs.forEach(log => {
    setTimeout(() => {
      decodeEffect(log.el, log.text, 15);
    }, log.delay);
  });
  ...
  // ========== 2. DECODER EFFECT =========="""
    ...
    # Use robust splitting to replace everything between boot and decoder
    ...
    if start_token in content and end_token in content:
        ...
        with open('js/extra-features.js', 'w', encoding='utf-8') as f:
            f.write(new_content)
        print("Updated preloader successfully.")
    else:
